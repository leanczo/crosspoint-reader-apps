            ...
            if (closeTag == "a") {
              ...
              if (!href.empty() && !cleanText.empty()) {
                ...
                if (uddgPos != std::string::npos) {
                  ...
                  std::string encoded = (end == std::string::npos) ? targetUrl.substr(start) : targetUrl.substr(start, end - start);
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Up)) {
      ...
      if (selectedIndex == 0) {
        auto keyboard = std::make_unique<KeyboardEntryActivity>(
            renderer, mappedInput, "DuckDuckGo Search", "", 40);
...
void DuckDuckGoActivity::render(RenderLock &&) {
  ...
  if (state == DDGState::Loading) {
    ...
    GUI.drawButtonMenu(
        ...
        offlineWebsites.size() + 1, selectedIndex,
        [this](int index) {
          if (index == 0)
            return std::string("[+ Search DuckDuckGo]");
          return offlineWebsites[index - 1];
        },
        [this](int index) {
          if (index == 0)
            return UIIcon::DuckDuckGo;
          return UIIcon::Book;
        },
        9);
